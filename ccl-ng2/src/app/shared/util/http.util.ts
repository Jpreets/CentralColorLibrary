...
import { DefaultHttpErrors, Exception, ExceptionRegistry } from '../constant';
...
export abstract class HttpUtil {
    ...
    */
    ...
    public static transformErrorResponse(err: any, errors: {[code: number]: Exception} = {}) {
        let errCode: number = HttpUtil.getResponseCode(err);
    ...
    public static getHttpDefaultError(code: number): Exception {
        let httpError: Exception = DefaultHttpErrors[code];
        if(!httpError) {
